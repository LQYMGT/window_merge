...
**/
PidginWindow *pwm_blist_get_convs(PidginBuddyList *gtkblist)
{
	if (gtkblist == NULL)
		return NULL;
...
**/
PidginBuddyList *pwm_convs_get_blist(PidginWindow *gtkconvwin)
{
	if (gtkconvwin == NULL)
		return NULL;
...
**/
void pwm_widget_replace(GtkWidget *child, GtkWidget *swap,
			GtkWidget *new_parent)
{
	GtkWidget *parent; /*< The parent of the widget to be replaced  */
	GValue val1 =
	    G_VALUE_INIT; /*< GtkContainer child property (manual get) */
	GValue val2 =
	    G_VALUE_INIT;      /*< GtkContainer child property (manual get) */
	...
	/* Sanity check: If not given a child or replacement, we've nothing to
	 * do. */
	...
	/* For a GtkBox parent, retrieve the current child widget properties. */
		...
		gtk_container_child_get_property(GTK_CONTAINER(parent), child,
						 "position", &val1);
		gtk_box_query_child_packing(GTK_BOX(parent), child, &expand,
	...
	if (should_unparent) {
		...
		gtk_container_remove(GTK_CONTAINER(gtk_widget_get_parent(swap)),
				     swap);
	}

	/* If no one is willing to adopt the orphaned child, it must be
	 * destroyed. */
	if (new_parent != NULL)
	...
	/* For a GtkPaned parent, add the new widget and restore child
	 * properties. */
	if (GTK_IS_PANED(parent)) {
		if (is_pane1)
	...
	/* For a GtkBox parent, add the new widget and restore child properties.
	   */
		...
		if (pack_type == GTK_PACK_START)
			gtk_box_pack_start(GTK_BOX(parent), swap, expand, fill,
					   padding);
		else
			gtk_box_pack_end(GTK_BOX(parent), swap, expand, fill,
					 padding);
		gtk_box_reorder_child(GTK_BOX(parent), swap,
				      g_value_get_int(&val1));
	}

	/* Remove the replacement's temporary reference that avoided
	 * destruction. */
	if (should_unparent)
